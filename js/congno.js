...
function tinhCongNo() {
  ...
  vanDonList.forEach((vd) => {
    ...
    let soNgayQuaHan =
      Math.floor((today - ngayTao) / (1000 * 60 * 60 * 24)) - 30;
...
async function loadCongNo() {
    ...
    try {
        ...
        const soKhachNo = list.length;
        const soKhachQuaHan = list.filter(item => item.soNgayQuaHanMax > 0).length;
...
function xemChiTietCongNo(tenKhach) {
  ...
  khach.chiTiet.forEach((vd) => {
    const quaHanText =
      vd.soNgayQuaHan > 0
        ? `<span class="overdue-badge">⚠ ${vd.soNgayQuaHan} ngày</span>`
        : "—";
    html += `<tr>
            <td>${vd.maVanDon}</td>
            <td>${vd.ngayTao}</td>
