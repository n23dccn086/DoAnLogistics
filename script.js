document.addEventListener('DOMContentLoaded', function() {
    ...
    // 4. CẤU HÌNH GIÁ
    let systemConfig = {
        normal: 15000, fragile: 25000, cold: 45000, threshold: 100, discount: 20
    };
    ...
    function updateDashboardStats() {
        const total = customerList.filter(i => i.status !== 'Đã hủy').reduce((s, i) => s + (parseInt(i.price.replace(/\D/g, '')) || 0), 0);
        ...
        if (document.getElementById('stat-revenue')) document.getElementById('stat-revenue').innerText = total.toLocaleString('vi-VN') + "đ";
