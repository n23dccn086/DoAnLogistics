...
const list = async (req, res) => {
  try {
    const { trangThai, trangThaiThanhToan, khachHang } = req.query;
    const filters = {};
    ...
    res
      .status(500)
      .json({ error: { code: "INTERNAL_ERROR", message: "Lỗi server" } });
  }
};
  ...
  try {
    ...
    if (!vanDon)
      return res
        .status(404)
        .json({
          error: { code: "NOT_FOUND", message: "Không tìm thấy vận đơn" },
        });
...
const create = async (req, res) => {
  try {
    ...
    const newVanDon = await VanDonModel.create({
      id,
      maBaoGia,
      khachHang,
      tuyen,
  ...
  try {
    ...
    if (!success)
      return res
        .status(404)
        .json({
          error: { code: "NOT_FOUND", message: "Vận đơn không tồn tại" },
        });
