...
class KhachHangModel {
    static async findAllActive() {
        const [rows] = await pool.query(`
            SELECT id, ten_cong_ty, ma_so_thue, nguoi_lien_he, 
    ...
    static async update(id, data) {
        ...
        const [result] = await pool.query(
            `UPDATE khach_hangs 
             SET ten_cong_ty = ?, ma_so_thue = ?, nguoi_lien_he = ?, 
                 so_dien_thoai = ?, email = ?, dia_chi = ?
             WHERE id = ?`,
    ...
    // ==================== XÓA CỨNG (DELETE) - Tạm thời dùng cách này ====================
    static async delete(id) {
        const [result] = await pool.query('DELETE FROM khach_hangs WHERE id = ?', [id]);
        return result.affectedRows > 0;
    }
}
